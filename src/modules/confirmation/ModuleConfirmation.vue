    ...
    >
      <template #dialogBody>
        <v-card-text ref="scrollableContent" class="py-0 px-5 px-md-0">
          ...
          >
            <b>Please double check everything.</b> Gwallet team will not be able
            to reverse your transaction once its submitted. You will still be
  ...
  computed: {
      ...
      if (!this.isBatch) {
        const parsedValue = this.tx.value
          ? this.tx.hasOwnProperty('toTxData')
            ? BigNumber(this.tx.toTxData.amount)
                .div(BigNumber(10).pow(this.sendCurrency.decimals))
                .toString()
    ...
    successBodyText() {
      return this.showSuccessSwap
        ? 'Once completed, the token amount will be deposited to your wallet. This should take a few minutes depending on how congested the The GSYS Chain Chain network is.'
        : 'Once completed, the token amount will be deposited to the address you provided. This should take a few minutes depending on how congested the The GSYS Chain network is.';
    }
  },
  watch: {
    error(newVal) {
